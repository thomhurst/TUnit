...
Write-Host "Running TUnit tests with the following configuration:" -ForegroundColor Cyan
...
function Run-Test {
    ...
    try {
        ...
        if ($success) {
            ...
            Write-Host "$TestName tests FAILED with exit code $LASTEXITCODE in $($stopwatch.Elapsed)" -ForegroundColor Red
        }
    } catch {
        $stopwatch.Stop()
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "AOT build FAILED with exit code $LASTEXITCODE" -ForegroundColor Red
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "SingleFile build FAILED with exit code $LASTEXITCODE" -ForegroundColor Red
        $results += [PSCustomObject]@{
            Name = "SingleFile"
            Success = $false
            Duration = [TimeSpan]::Zero
...
if ($passedCount -eq $totalCount) {
    Write-Host "`nAll tests PASSED! ($passedCount/$totalCount)" -ForegroundColor Green
    exit 0
} else {
    Write-Host "`n$passedCount/$totalCount tests PASSED" -ForegroundColor Red
