...
public class TestMetadataGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateMethodMetadataHelper(StringBuilder sb, TestMethodInfo testInfo, int index)
    {
        ...
        {
            sb.AppendLine("                new ParameterMetadata");
            sb.AppendLine("                {");
            sb.AppendLine($"                    Name = \"{param.Name}\",");
            sb.AppendLine($"                    Type = typeof({GetFullTypeName(param.Type)}),");
            sb.AppendLine($"                    Ordinal = {param.Ordinal},");
            sb.AppendLine($"                    HasDefaultValue = {(param.HasExplicitDefaultValue ? "true" : "false")},");
            if (param.HasExplicitDefaultValue)
            {
                sb.AppendLine($"                    DefaultValue = {FormatValue(param.ExplicitDefaultValue)},");
            }
            else
            {
