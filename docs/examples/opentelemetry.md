...
### Option A: Zero-config (`TUnit.OpenTelemetry`)[​](#option-a-zero-config-tunitopentelemetry "Direct link to option-a-zero-config-tunitopentelemetry")
...
#### Customizing (add exporters, processors, resources)[​](#customizing-add-exporters-processors-resources "Direct link to Customizing (add exporters, processors, resources)")
...
#### Environment switches[​](#environment-switches "Direct link to Environment switches")
...
#### Coexistence with hand-rolled setup[​](#coexistence-with-hand-rolled-setup "Direct link to Coexistence with hand-rolled setup")
...
### Option B: Manual (full control)[​](#option-b-manual-full-control "Direct link to Option B: Manual (full control)")
...
### Option C: Raw `ActivityListener` (no SDK dependency)[​](#option-c-raw-activitylistener-no-sdk-dependency "Direct link to option-c-raw-activitylistener-no-sdk-dependency")
...
### Why `[Before(TestDiscovery)]`?[​](#why-beforetestdiscovery "Direct link to why-beforetestdiscovery")
...
## Activity Sources[​](#activity-sources "Direct link to Activity Sources")
...
### Standard OTel Attributes[​](#standard-otel-attributes "Direct link to Standard OTel Attributes")
...
### TUnit-Specific Attributes[​](#tunit-specific-attributes "Direct link to TUnit-Specific Attributes")
...
## Using with Jaeger, Zipkin, or OTLP[​](#using-with-jaeger-zipkin-or-otlp "Direct link to Using with Jaeger, Zipkin, or OTLP")

Swap the exporter in the setup code above. Each exporter needs its own NuGet package.

### OTLP (works with Jaeger, Tempo, Honeycomb, etc.)[​](#otlp-works-with-jaeger-tempo-honeycomb-etc "Direct link to OTLP (works with Jaeger, Tempo, Honeycomb, etc.)")
...
## Test Context Correlation via Activity Baggage[​](#test-context-correlation-via-activity-baggage "Direct link to Test Context Correlation via Activity Baggage")
...
### "I see one trace per test instead of one trace per class"[​](#i-see-one-trace-per-test-instead-of-one-trace-per-class "Direct link to \"I see one trace per test instead of one trace per class\"")
...
### "Spans from test A are showing up under test B"[​](#spans-from-test-a-are-showing-up-under-test-b "Direct link to \"Spans from test A are showing up under test B\"")
...
### "My SUT spans show no parent / appear orphaned"[​](#my-sut-spans-show-no-parent--appear-orphaned "Direct link to \"My SUT spans show no parent / appear orphaned\"")
...
### "My HTTP calls don't carry the test trace"[​](#my-http-calls-dont-carry-the-test-trace "Direct link to \"My HTTP calls don't carry the test trace\"")
...
### "No spans show up in my exporter at all"[​](#no-spans-show-up-in-my-exporter-at-all "Direct link to \"No spans show up in my exporter at all\"")
...
## HTML Report Integration[​](#html-report-integration "Direct link to HTML Report Integration")
