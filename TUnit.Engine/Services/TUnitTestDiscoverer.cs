    ...
    TestsExecutor testsExecutor)
{
    ...
    public async Task<IReadOnlyCollection<DiscoveredTest>> GetTestsAsync(ExecuteRequestContext context, CancellationToken cancellationToken = default)
    {
        return _cachedTests ??= await testsConstructor.GetTestsAsync(context, cancellationToken);
    }
    ...
    public async Task<GroupedTests> FilterTests(ExecuteRequestContext context, CancellationToken cancellationToken)
    {
        ...
        var allDiscoveredTests = await GetTestsAsync(context, cancellationToken);
