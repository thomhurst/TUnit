...
Write-Host "Running Source Generation tests..." -ForegroundColor Yellow
...
try {
    ...
    if ($success) {
        Write-Host "Source Generation tests PASSED in $($stopwatch.Elapsed)" -ForegroundColor Green
    } else {
        Write-Host "Source Generation tests completed with exit code $LASTEXITCODE in $($stopwatch.Elapsed)" -ForegroundColor Yellow
    }
