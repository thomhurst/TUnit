...
public class DependsOnTests3 : TestModule
{
    protected override async Task<TestResult?> ExecuteAsync(IPipelineContext context, CancellationToken cancellationToken)
    {
        ...
        var test3Start = trxReport.UnitTestResults.FirstOrDefault(x => x.TestName!.StartsWith("Test3"))?.StartTime!.Value ?? throw new Exception($"Test1 not found: {JsonSerializer.Serialize(trxReport)}");
