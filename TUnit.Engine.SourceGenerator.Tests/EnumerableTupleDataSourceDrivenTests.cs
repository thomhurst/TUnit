...
internal class EnumerableTupleDataSourceDrivenTests : TestsBase<TestsGenerator>
{
    ...
    public Task Test() => RunTest(Path.Combine(Git.RootDirectory.FullName,
        ...
        generatedFiles =>
        {
            ...
            Assert.That(generatedFiles[0], Does.Contain("TestMethodFactory = (classInstance, cancellationToken) => RunHelpers.RunWithTimeoutAsync(() => classInstance.DataSource_TupleMethod(methodArg1, methodArg2, methodArg3), cancellationToken),"));
            ...
            Assert.That(generatedFiles[1], Does.Contain("var (methodArg1, methodArg2, methodArg3) = methodArg0;"));
            Assert.That(generatedFiles[1], Does.Contain("TestMethodArguments = [methodArg1, methodArg2, methodArg3],"));
            Assert.That(generatedFiles[1], Does.Contain("DisplayName = $\"DataSource_TupleMethod({methodArg1}, {methodArg2}, {methodArg3})\","));
            Assert.That(generatedFiles[1], Does.Contain("TestMethodFactory = (classInstance, cancellationToken) => RunHelpers.RunWithTimeoutAsync(() => classInstance.DataSource_TupleMethod(methodArg1, methodArg2, methodArg3), cancellationToken),"));
        });
}