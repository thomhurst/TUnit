...
internal static class MockBridgeBuilder
{
    public static string Build(MockTypeModel model)
    {
        ...
        var staticEngineTypeName = MockImplBuilder.GetGeneratedTypeName($"{safeName}StaticEngine", model);
        ...
        using (writer.Block($"namespace {mockNamespace}"))
        {
            ...
            writer.AppendLine();
            BuildStaticEngineClass(writer, model, staticEngineTypeName);
        }

        return writer.ToString();
    }

    private static void BuildBridgeInterface(CodeWriter writer, MockTypeModel model, string safeName, string staticEngineTypeName)
    {
        ...
        using (writer.Block($"{model.Visibility} interface {safeName}Mockable{typeParams} : {model.FullyQualifiedName}{constraints}"))
        {
            bool first = true;
            ...
            foreach (var prop in model.Properties)
            {
                ...
                if (!first) writer.AppendLine();
                first = false;
                GenerateStaticPropertyDim(writer, prop, staticEngineTypeName);
            }
    ...
    private static void BuildStaticEngineClass(CodeWriter writer, MockTypeModel model, string staticEngineTypeName)
    {
        var constraints = MockImplBuilder.GetConstraintClauses(model);
        writer.AppendLine("[global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]");
        using (writer.Block($"internal static class {staticEngineTypeName}{constraints}"))
        {
