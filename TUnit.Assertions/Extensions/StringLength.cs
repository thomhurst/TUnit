...
public class StringLength
{
    ...
    {
        return _valueSource.RegisterAssertion(new DelegateExpectedValueAssertCondition<string, int>(expected, (actual, _, self) =>
            {
                if (actual is null)
                {
    ...
    public InvokableValueAssertionBuilder<string> Zero =>
        _valueSource.RegisterAssertion(new DelegateExpectedValueAssertCondition<string, int>(0,
                (@string, _, self) =>
                {
                    if (@string is null)
                    {
    ...
    public InvokableValueAssertionBuilder<string> Positive =>
        _valueSource.RegisterAssertion(new DelegateExpectedValueAssertCondition<string, int>(default, (@string, _, self) =>
            {
                if (@string is null)
                {
                    self.OverriddenMessage = "Actual string is null";
    ...
    {
        return _valueSource.RegisterAssertion(new DelegateExpectedValueAssertCondition<string, int>(expected,
            (@string, _, self) =>
            {
                if (@string is null)
                {
    ...
    {
        return _valueSource.RegisterAssertion(new DelegateExpectedValueAssertCondition<string, int>(expected, (@string, _, self) =>
            {
                if (@string is null)
                {
                    self.OverriddenMessage = "Actual string is null";
