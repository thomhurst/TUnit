...
public static class TestApplicationBuilderExtensions
{
    public static void AddTUnit(this ITestApplicationBuilder testApplicationBuilder)
    {
        ...
        testApplicationBuilder.TestHost.AddTestSessionLifetimeHandler(serviceProvider =>
        {
            ...
            if (commandLineOptions.TryGetOptionArgumentList(HtmlReporterCommandProvider.TUnitReportHtmlFilename, out var pathArgs))
            {
                htmlReporter.SetOutputPath(Helpers.PathValidator.ValidateAndNormalizePath(pathArgs[0], HtmlReporterCommandProvider.TUnitReportHtmlFilename));
            }
            ...
            // Set results directory as specified by --results-directory,
            // so it can be used in the default output path if --tunit-report-html-filename is not provided
